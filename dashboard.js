...
async function gmailThreadsAll(token, query, max=60){
  const all = [];
  let next = undefined;
  do {
...
function extractBodyFromPayload(payload) {
  if (!payload) return '';
  const partsStack = [payload];
  let text = '';
  while (partsStack.length) {
    const part = partsStack.pop();
    if (!part) continue;
    if (part.parts && part.parts.length) {
      partsStack.push(...part.parts);
    }
...
async function syncAndRender() {
  try {
    ...
    const threads = await gmailThreadsAll(token, query || 'subject:(application OR applied)', 60);
    console.log('Fetched threads:', threads.length);
    
    // Ensure parsers are loaded
    if (!window.parsers) {
      console.log('Parsers not loaded, waiting up to 5s...');
      await new Promise((resolve) => {
        const start = Date.now();
        const checkParsers = () => {
          if (window.parsers || Date.now() - start > 5000) {
            if (!window.parsers) console.warn('Proceeding without parsers after timeout');
            resolve();
          } else {
    ...
    for (const t of threads){
      ...
      try {
        ...
        const msg = (detail.messages && detail.messages.length) ? detail.messages[detail.messages.length - 1] : null;
        const headers = (msg?.payload?.headers) || [];
        from = headers.find(h=>h.name==='From')?.value || '';
        subject = headers.find(h=>h.name==='Subject')?.value || '';
        // Prefer the latest message body in the thread
        const bodyText = extractBodyFromPayload(msg?.payload) || (t.snippet || '');
        // Get the actual email date from headers
        const dateHeader = headers.find(h=>h.name==='Date')?.value;
        if (dateHeader) {
          emailDate = new Date(dateHeader).toISOString();
        }
        // Overwrite snippet with real text to improve parsing
        t.snippet = bodyText.slice(0, 5000);
      } catch (e) {
      ...
      // Extra status detection across all emails using GenericParser rules
      try {
        if (window.parsers) {
          const generic = window.parsers.find(p => p && p.determineStatus && p.constructor && p.constructor.name === 'GenericParser');
          if (generic) {
            const detected = generic.determineStatus({ subject, body: t.snippet || '' }) || 'applied';
            const normalized = (detected || 'applied').toLowerCase();
            const allowed = ['applied','viewed','interview','offer','rejected','ghost'];
            if (allowed.includes(normalized) && normalized !== (app.status || 'applied').toLowerCase()) {
              app.status = normalized;
              if (!app.statusHistory) app.statusHistory = [];
              app.statusHistory.push({ status: normalized, at: new Date().toISOString() });
            }
          }
        }
      } catch (e) { /* ignore */ }
      ...
      // Batch render every 20 items to avoid "feels stuck" UX on large inboxes
      if (processed % 20 === 0) {
        try {
          const statsMid = {
