...
async function clearIdentityCachedTokenIfAny() {
  // Try remove via stored token first
  try {
    const stored = await getStoredToken();
    if (stored?.accessToken) {
      await new Promise((resolve) => {
        chrome.runtime.sendMessage({ type: 'removeToken', token: stored.accessToken }, () => resolve());
      });
    }
  } catch (_) {}
  // Also try to fetch a cached token silently then remove it
  try {
    await new Promise((resolve) => {
      chrome.identity.getAuthToken({ interactive: false }, (t) => {
        if (t) {
          chrome.runtime.sendMessage({ type: 'removeToken', token: t }, () => resolve());
        } else {
          resolve();
        }
      });
...
async function clearAllTokens() {
  setStatus('Clearing token cache...');
  try {
    await clearIdentityCachedTokenIfAny();
    await clearStoredToken();
    setStatus('Token cache cleared');
  } catch (e) {
    setStatus('Token cache cleared (with warnings)');
  }
  show(signinBtn);
  hide(signoutBtn);
  hide(fetchBtn);
...
clearCacheBtn.addEventListener('click', clearAllTokens);
